            ...
            try
            {
                await Task.Run(async () =>
                {
                    for (int i = 0; i < itens.Count; i++)
                    {
                        ...
                        try
                        {
                            // Processar entrada: pode ser caminho XML ou chave/número
                            NfceInfo info;
                            if (File.Exists(entrada))
                            {
                                ...
                                // Define status direto aqui, sem depender do XmlParserDetalhe
                                if (!string.IsNullOrEmpty(info.ChaveAcesso) && info.ChaveAcesso.Length == 44)
                                {
                                    info.StatusSefaz = StatusConsulta.ComPagamento;
                                    info.Observacao = "✔ EMITIDA";
                                }
                            }
                            ...
                            // Consulta SEFAZ só para pegar a data de pagamento, SEM alterar o status
                            if (!string.IsNullOrEmpty(info.ChaveAcesso))
                            {
                                ...
                                // NÃO sobrescreve info.StatusSefaz nem info.Observacao
                            }
                            else
                            {
